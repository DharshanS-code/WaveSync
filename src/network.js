...
import { ClockSync } from './sync-engine.js';
...
export class SignalClient extends EventTarget {
  ...
  _stopClock() { if (this._clockT) clearInterval(this._clockT); this._clockT = null; }
  ...
  _handle(m) {
    switch (m.t) {
      ...
      case 'time': {
        const t2 = Date.now();
        this.clock.addSample(m.t0, m.t1, t2);
        this.dispatchEvent(new CustomEvent('clock', { detail: { offset: this.clock.offset, rtt: this.clock.rtt } }));
        break;
      }
      case 'peer-join':
      case 'peer-leave':
